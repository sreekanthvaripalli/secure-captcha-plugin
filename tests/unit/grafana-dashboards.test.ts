...
describe('Grafana Dashboards', () => {
  ...
  describe('Performance Dashboard', () => {
    ...
    it('should have Prometheus datasource', () => {
      ...
      const hasPrometheusDatasource = panels.some(
        (panel: any) =>
    ...
    it('should have request rate panel', () => {
      const panels = performanceDashboard.panels;
      const hasRequestRate = panels.some(
        (panel: any) =>
          panel.title === 'Request Rate' ||
          (panel.targets &&
            panel.targets.some(
              (target: any) => target.expr && target.expr.includes('captcha_http_requests_total')
            ))
      );
    ...
    it('should have error rate panel', () => {
      const panels = performanceDashboard.panels;
      const hasErrorRate = panels.some(
        (panel: any) =>
          panel.title === 'Error Rate' ||
          (panel.targets &&
            panel.targets.some(
              (target: any) => target.expr && target.expr.includes('captcha_http_errors_total')
            ))
      );
    ...
    it('should have throughput panel', () => {
      const panels = performanceDashboard.panels;
      const hasThroughput = panels.some(
        (panel: any) =>
    ...
    it('should have captcha generation time panel', () => {
      const panels = performanceDashboard.panels;
      const hasGenerationTime = panels.some(
        (panel: any) =>
    ...
    it('should have captcha validation time panel', () => {
      const panels = performanceDashboard.panels;
      const hasValidationTime = panels.some(
        (panel: any) =>
    ...
    it('should have cache hit rate panel', () => {
      const panels = performanceDashboard.panels;
      const hasCacheHitRate = panels.some(
        (panel: any) =>
          (panel.title && panel.title.includes('Cache Hit Rate')) ||
          (panel.targets &&
            panel.targets.some(
              (target: any) => target.expr && target.expr.includes('captcha_cache_hits_total')
            ))
      );
    ...
    it('should have memory usage panel', () => {
      const panels = performanceDashboard.panels;
      const hasMemoryUsage = panels.some(
        (panel: any) =>
          (panel.title && panel.title.includes('Memory')) ||
          (panel.targets &&
            panel.targets.some(
              (target: any) => target.expr && target.expr.includes('captcha_memory_usage_bytes')
            ))
      );
    ...
    it('should have CPU usage panel', () => {
      const panels = performanceDashboard.panels;
      const hasCpuUsage = panels.some(
        (panel: any) =>
          (panel.title && panel.title.includes('CPU')) ||
          (panel.targets &&
            panel.targets.some(
              (target: any) => target.expr && target.expr.includes('captcha_cpu_usage_seconds')
            ))
      );
  ...
  describe('Security Dashboard', () => {
    ...
    it('should have security events panel', () => {
      ...
      const hasSecurityEvents = panels.some(
        (panel: any) =>
          (panel.title && panel.title.includes('Security Events')) ||
          (panel.targets &&
            panel.targets.some(
              (target: any) => target.expr && target.expr.includes('captcha_security_events_total')
            ))
      );
    ...
    it('should have rate limit hits panel', () => {
      const panels = securityDashboard.panels;
      const hasRateLimit = panels.some(
        (panel: any) =>
          (panel.title && panel.title.includes('Rate Limit')) ||
          (panel.targets &&
            panel.targets.some(
              (target: any) => target.expr && target.expr.includes('captcha_rate_limit_hits_total')
            ))
      );
    ...
    it('should have validation results panel', () => {
      const panels = securityDashboard.panels;
      const hasValidationResults = panels.some(
        (panel: any) =>
          (panel.title && panel.title.includes('Validation Results')) ||
          (panel.targets &&
            panel.targets.some(
              (target: any) => target.expr && target.expr.includes('captcha_validation_total')
            ))
      );
    ...
    it('should have failed validations rate panel', () => {
      const panels = securityDashboard.panels;
      const hasFailedValidations = panels.some(
        (panel: any) =>
          (panel.title && panel.title.includes('Failed Validations')) ||
          (panel.targets &&
            panel.targets.some(
              (target: any) => target.expr && target.expr.includes('result=\\"incorrect\\"')
            ))
      );
    ...
    it('should have session deletion reasons panel', () => {
      const panels = securityDashboard.panels;
      const hasSessionDeletions = panels.some(
        (panel: any) =>
          (panel.title && panel.title.includes('Session Deletion')) ||
          (panel.targets &&
            panel.targets.some(
              (target: any) => target.expr && target.expr.includes('captcha_sessions_deleted_total')
            ))
      );
    ...
    it('should have generation failures panel', () => {
      const panels = securityDashboard.panels;
      const hasGenerationFailures = panels.some(
        (panel: any) =>
          (panel.title && panel.title.includes('Generation Failures')) ||
          (panel.targets &&
            panel.targets.some(
              (target: any) => target.expr && target.expr.includes('status=\\"failure\\"')
            ))
      );
  ...
  describe('Business Dashboard', () => {
    ...
    it('should have captcha types usage panel', () => {
      ...
      const hasCaptchaTypes = panels.some(
        (panel: any) =>
          (panel.title && panel.title.includes('Captcha Types')) ||
          (panel.targets &&
            panel.targets.some((target: any) => target.expr && target.expr.includes('by (type)')))
      );
    ...
    it('should have difficulty distribution panel', () => {
      const panels = businessDashboard.panels;
      const hasDifficulty = panels.some(
        (panel: any) =>
          (panel.title && panel.title.includes('Difficulty')) ||
          (panel.targets &&
            panel.targets.some(
              (target: any) => target.expr && target.expr.includes('by (difficulty)')
            ))
      );
    ...
    it('should have success rate by type panel', () => {
      const panels = businessDashboard.panels;
      const hasSuccessRate = panels.some(
        (panel: any) =>
          (panel.title && panel.title.includes('Success Rate')) ||
          (panel.targets &&
            panel.targets.some(
              (target: any) => target.expr && target.expr.includes('result=\\"correct\\"')
            ))
      );
    ...
    it('should have text captchas stat panel', () => {
      const panels = businessDashboard.panels;
      const hasTextCaptchas = panels.some(
        (panel: any) =>
          panel.title === 'Text Captchas' ||
          (panel.targets &&
            panel.targets.some(
              (target: any) => target.expr && target.expr.includes('type=\\"text\\"')
            ))
      );
    ...
    it('should have math captchas stat panel', () => {
      const panels = businessDashboard.panels;
      const hasMathCaptchas = panels.some(
        (panel: any) =>
          panel.title === 'Math Captchas' ||
          (panel.targets &&
            panel.targets.some(
              (target: any) => target.expr && target.expr.includes('type=\\"math\\"')
            ))
      );
    ...
    it('should have logic captchas stat panel', () => {
      const panels = businessDashboard.panels;
      const hasLogicCaptchas = panels.some(
        (panel: any) =>
          panel.title === 'Logic Captchas' ||
          (panel.targets &&
            panel.targets.some(
              (target: any) => target.expr && target.expr.includes('type=\\"logic\\"')
            ))
      );
    ...
    it('should have image captchas stat panel', () => {
      const panels = businessDashboard.panels;
      const hasImageCaptchas = panels.some(
        (panel: any) =>
          panel.title === 'Image Captchas' ||
          (panel.targets &&
            panel.targets.some(
              (target: any) => target.expr && target.expr.includes('type=\\"image\\"')
            ))
      );
  ...
  describe('Dashboard Consistency', () => {
    ...
    it('should have unique dashboard UIDs', () => {
      const uids = [performanceDashboard.uid, securityDashboard.uid, businessDashboard.uid];
  ...
  describe('Dashboard Metrics Coverage', () => {
    ...
    it('should cover all Prometheus metrics', () => {
      ...
      const allExpressions = allDashboards
        .flatMap(dashboard =>
